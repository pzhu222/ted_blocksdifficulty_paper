...
```
## Exploratory: Tsimane' Children fisher exact test

```{r}

#just stability trial for now

df.e3childfc |> 
  filter(trial == "Stability") |> 
  mutate(median_age = ifelse(age < median(age), "Younger", "Older"),
         median_age = as.factor(median_age)) |> 
  select(correct, median_age) |> 
  #make two dimensional contingency table for fisher exact test, with median age as columns and correct/incorrect as rows
  table() |> 
  fisher.test()
